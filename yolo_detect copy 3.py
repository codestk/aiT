...
class Worker(QThread):
    ...
    # MODIFIED: เพิ่ม is_image_source เข้ามา
    def __init__(self, model_path, source, initial_thresh, is_image_source):
        super().__init__()
        self.model_path = model_path
        self.source = source
        self.threshold = initial_thresh
        self.is_image_source = is_image_source # ADDED: เก็บสถานะว่าเป็นรูปภาพหรือไม่
    ...
    def run(self):
        try:
            ...
            # MODIFIED: แยก Logic ระหว่างรูปภาพกับวิดีโอ
    ...
    def process_single_image(self, model, labels):
        """ฟังก์ชันสำหรับประมวลผลรูปภาพเดียว"""
    ...
    def process_video_stream(self, model, labels):
        """ฟังก์ชันสำหรับประมวลผลวิดีโอหรือเว็บแคม (โค้ดเดิม)"""
        try:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # --- เชื่อมต่อ Signals กับ Slots (เหมือนเดิม) ---
    ...
    def start_detection(self):
        ...
        # ADDED: ตรวจสอบว่าเป็นไฟล์รูปภาพหรือไม่
        image_extensions = ['.jpg', '.jpeg', '.png', '.bmp']
        is_image = any(source.lower().endswith(ext) for ext in image_extensions)
        
        # MODIFIED: สำหรับรูปภาพ ปุ่ม Stop ไม่จำเป็นต้องกด
        if not is_image:
            self.stop_button.setEnabled(True)
        
        self.start_button.setEnabled(False)
        
        # MODIFIED: ส่ง is_image เข้าไปใน Worker
    ...
    def stop_detection(self):
        ...
        if self.worker:
            self.worker.stop()
        # ไม่ต้องรอ finished signal เพราะผู้ใช้เป็นคนสั่งหยุดเอง
        self.stop_button.setEnabled(False)
        self.start_button.setEnabled(True)

    def detection_finished(self):
        # ฟังก์ชันนี้จะถูกเรียกเมื่อ thread ทำงานเสร็จ (จบวิดีโอ หรือประมวลผลรูปเสร็จ)
        self.status_label.setText("Finished.")
        if not self.worker.is_image_source: # ถ้าเป็นวิดีโอถึงจะแสดงข้อความนี้
