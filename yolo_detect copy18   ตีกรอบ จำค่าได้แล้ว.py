...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        if self.roi:
            x, y, w, h = self.roi
            self.image_display_label.selected_roi_rect = QRect(x, y, w, h)
    ...
    def set_roi(self, selection_rect):
        ...
        with self.worker.lock:
            latest_frame = self.worker.latest_frame.copy() if hasattr(self.worker, 'latest_frame') and self.worker.latest_frame is not None else None
        
        if latest_frame is None: return
        
        h_orig, w_orig = latest_frame.shape[0], latest_frame.shape[1]
        ...
        # MODIFIED: Save ROI to config immediately after selection
    ...
    def clear_roi(self):
        ...
        # MODIFIED: Save cleared ROI to config
        self.save_config_with_roi()
