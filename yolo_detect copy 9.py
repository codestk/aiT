...
class Worker(QThread):
    ...
    def process_single_image(self, model, labels):
        ...
        original_frame = frame.copy() # ADDED: สร้างสำเนาของภาพต้นฉบับ
        object_found = False # ADDED: Flag ตรวจสอบว่าเจอวัตถุหรือไม่
        ...
        for i in range(len(detections)):
            ...
            if conf > self.threshold:
                object_found = True # ADDED: ตั้งค่า Flag
        ...
        # MODIFIED: บันทึกภาพก็ต่อเมื่อตรวจเจอวัตถุ
        if object_found:
            base, ext = os.path.splitext(self.source)
            output_path_detected = f"{base}_detected{ext}"
            output_path_original = f"{base}_original{ext}"
            
            cv2.imwrite(output_path_detected, frame)
            cv2.imwrite(output_path_original, original_frame)
            self.status_update.emit(f"Detection saved to: {base}_...")
        else:
    ...
    def process_video_stream(self, model, labels):
        ...
        while self._is_running:
            ...
            original_frame = frame.copy() # ADDED: สร้างสำเนาภาพต้นฉบับทุกเฟรม
            object_found_in_frame = False # ADDED: Flag สำหรับเฟรมปัจจุบัน
            ...
            for i in range(len(detections)):
                conf = detections[i].conf.item()
                if conf > self.threshold:
                    object_found_in_frame = True # ADDED: ตั้งค่า Flag เมื่อเจอ
                    object_count += 1
                    xyxy = detections[i].xyxy.cpu().numpy().squeeze().astype(int)
                    xmin, ymin, xmax, ymax = xyxy
                    classidx = int(detections[i].cls.item())
            ...
            # ADDED: ถ้าเจอวัตถุในเฟรมนี้ ให้บันทึกภาพทั้งสองเวอร์ชัน
    ...
    # ADDED: ฟังก์ชันใหม่สำหรับบันทึกภาพที่ตรวจเจอโดยเฉพาะ
    def save_detection_images(self, original_frame, detected_frame):
        """บันทึกภาพต้นฉบับและภาพที่ตรวจเจอลงในโฟลเดอร์ outputs/detections"""
        output_dir = 'outputs/detections'
        os.makedirs(output_dir, exist_ok=True)
        # ใช้ timestamp ที่มีความละเอียดสูงเพื่อป้องกันชื่อไฟล์ซ้ำ
